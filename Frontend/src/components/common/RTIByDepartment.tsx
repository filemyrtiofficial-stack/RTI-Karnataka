...
const RTIByDepartmentComponent: React.FC<RTIByDepartmentProps> = ({ state = 'telangana' }) => {
  ...
  const departments = isKarnataka ? karnatakaDepartments : telanganaDepartments;
  const stateName = isKarnataka ? 'Karnataka' : 'Telangana';
  ...
  // Render Karnataka and Telangana with same multi-column text layout
  return (
    <>
      <section className={isKarnataka ? 'bg-primary-50 pb-12 md:pb-16 lg:pb-20' : 'bg-white py-12 md:py-16 lg:py-20'} aria-label={`RTI Services by ${stateName} Department`}>
        <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8">
          {/* Section Header */}
          <div className="text-center mb-3">
            <h2 className="mb-2 text-[32px] md:text-[36px] font-semibold text-gray-900">
              {isKarnataka ? 'RTI TEMPLATES FOR KARNATAKA DEPARTMENTS' : `RTI for ${stateName} Government Departments`}
            </h2>
            {isKarnataka && (
          ...
          {/* Compact Multi-Column Layout - Same for Karnataka and Telangana */}
          <nav aria-label="RTI Department Navigation" className="mt-8">
            <div className={isKarnataka ? 'bg-white rounded-lg px-6 md:px-8 lg:px-10 py-6' : ''}>
